...
exports.getChildModelId = getChildModelId;
exports.initChildStory = initChildStory;
exports.generateChildStoryContent = generateChildStoryContent;
exports.createChildStory = createChildStory;
exports.initContinueChildStory = initContinueChildStory;
exports.generateNextChildChapter = generateNextChildChapter;
exports.continueChildStory = continueChildStory;
...
function getChildModelId(options) {
    if (options.gradeLevel !== null && options.gradeLevel !== undefined) {
        return options.gradeLevel <= constants_1.ELEMENTARY_MAX_GRADE ? CHILD_STORY_HAIKU_MODEL_ID : CHILD_STORY_SONNET_MODEL_ID;
    }
    if (options.age !== null && options.age !== undefined) {
        return options.age <= constants_1.ELEMENTARY_MAX_AGE ? CHILD_STORY_HAIKU_MODEL_ID : CHILD_STORY_SONNET_MODEL_ID;
    }
    if (options.readingLevel) {
        const level = options.readingLevel.toUpperCase();
        return constants_1.ELEMENTARY_GRL_LEVELS.includes(level) ? CHILD_STORY_HAIKU_MODEL_ID : CHILD_STORY_SONNET_MODEL_ID;
    }
    return CHILD_STORY_HAIKU_MODEL_ID;
}
function getChildAgeBand(options) {
    if (options.gradeLevel !== null && options.gradeLevel !== undefined) {
        if (options.gradeLevel < 0)
            return 'prek';
        if (options.gradeLevel <= 2)
            return 'early-elementary';
        if (options.gradeLevel <= 5)
            return 'upper-elementary';
        if (options.gradeLevel <= 8)
...
function getChildContentGuidelines(ageBand, readingPurpose) {
...
function getChildVocabularyGuidance(ageBand) {
    switch (ageBand) {
        case 'prek':
...
function getChildChapterLength(ageBand) {
    switch (ageBand) {
        case 'prek':
...
function buildChildOutlineSystemPrompt(config, targetNodeCount, ageBand, readingPurpose) {
...
function buildChildOutlineUserContent(options, config) {
...
function buildChildRootChapterSystemPrompt(config, outline, targetNodeCount, ageBand, readingPurpose) {
...
function buildChildRootChapterUserContent(options, config) {
    const premise = options.customPrompt
        ? `Custom request: "${options.customPrompt}"`
...
function buildChildContinuationSystemPrompt(config, bible, outline, currentChapterIndex, targetNodeCount, stage, isForceEnding, ageBand, readingPurpose) {
...
function buildChildContinuationUserContent(recentNodes, chosenLabel, userHint) {
...
async function initChildStory(options) {
...
async function generateChildStoryContent(storyId, options) {
    ...
    try {
        ...
        const outlineSystemPrompt = buildChildOutlineSystemPrompt(config, targetNodeCount, ageBand, options.readingPurpose);
        const outlineUserContent = buildChildOutlineUserContent(options, config);
        const t0Outline = Date.now();
        const outlineResponse = await invokeBedrockYAML(childModelId, outlineSystemPrompt, outlineUserContent, storyId, options.profileId);
        ...
        const rootSystemPrompt = buildChildRootChapterSystemPrompt(config, outline, targetNodeCount, ageBand, options.readingPurpose);
        const rootUserContent = buildChildRootChapterUserContent(options, config);
        const t0Root = Date.now();
        const rootResponse = await invokeBedrockYAML(childModelId, rootSystemPrompt, rootUserContent, storyId, options.profileId);
...
async function generateNextChildChapter(storyId, profileId, options, userHint) {
    ...
    try {
        ...
        const systemPrompt = buildChildContinuationSystemPrompt(story.config, story.bible, story.outline, newChapterIndex, story.targetNodeCount, stage, isForceEnding, ageBand, options.readingPurpose);
        const userContent = buildChildContinuationUserContent(pathToNode, chosenChoice.label, userHint);
        const t0Chapter = Date.now();
        const aiResponse = await invokeBedrockYAML(childModelId, systemPrompt, userContent, storyId, profileId);
...
async function continueChildStory(options, createOptions) {
